...
def flash_attention(q, k, v, causal=True):
    ...
    if causal:
        # Create a causal mask (upper triangular with diagonal zero)
...
def apply_rotary_emb(x, cos, sin, interleaved=False):
    ...
    cos, sin: tensors of shape [seq_length, d_rot] where d_rot is typically head_dim//2.
    Rotates only the first d_rot components.
    """
    d_rot = cos.shape[-1]
    x_rot = x[..., :d_rot]
    ...
    # If x has extra dimensions beyond d_rot, leave them unchanged.
    if x.shape[-1] > d_rot:
...
def get_cos_sin(seq_length, head_dim, base=500000.0):
    """
    Generates cosine and sine matrices for rotary embeddings.
    """
    assert head_dim % 2 == 0
    theta = 1.0 / (base ** (torch.arange(0, head_dim, 2, dtype=torch.int64)
                               .float().to('cpu') / head_dim))
    # Use bfloat16 for consistency with some implementations.
...
class Attention(nn.Module):
    ...
    def forward(self, x, cos, sin, attention_mask=None, position_ids=None):
        ...
        k = self.k_proj(x)  # [B, T, num_key_values*head_dim]
        v = self.v_proj(x)  # [B, T, num_key_values*head_dim]
        ...
        k = k.transpose(1, 2)  # [B, num_key_values, T, head_dim]
        v = v.view(batch_size, seq_length, self.num_local_kv_heads, self.head_dim).transpose(1, 2)  # [B, num_key_values, T, head_dim]
     
        k = k.repeat_interleave(self.num_local_heads // self.num_local_kv_heads, dim=1)  # [B, num_heads, T, head_dim]
        v = v.repeat_interleave(self.num_local_heads // self.num_local_kv_heads, dim=1)  # [B, num_heads, T, head_dim]
        
        # If decoding, q may have a different sequence length; use causal masking only when needed.
        causal = True if q.size(2) == k.size(2) else False

        out = flash_attention(q, k, v, causal=causal)  # [B, T, num_heads, head_dim]
        out = out.reshape(batch_size, seq_length, self.num_local_heads * self.head_dim)  # [B, T, hidden_dim]
...
class DecoderLayer(nn.Module):
    # Structure: RMSNorm -> Attention -> Residual -> RMSNorm -> MLP -> Residual
    def __init__(self, config, layer_idx):
        super().__init__()
